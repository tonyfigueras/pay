    ...
    public function login(LoginRequest $request)
	{
		...
		if (!$verifyAuth) {
			...
			return response()->json($jsonMessage, 400);
								...
								/* 'alumno', 'alumno.curso',*/
		$user = User::with(['personal_data',  'wallet'])
			->find(Auth::id());
		...
		if ($block) {
		 ...
		 // Determinar habilidades según privilegio
		$abilities = match($user->privilegio) {
			'A-' => ['*'],         // Admin - todos los permisos
			'P-' => ['profesor'],  // Profesor
			default => ['user']    // Usuario normal
		};
		// Token
		...
		$accessToken = $user->createToken(
			...
			$abilities,
			$request->checkbox ? null : now()->addHours(8)
		)->plainTextToken;
